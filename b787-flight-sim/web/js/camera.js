...
export const VIEWS = ['cockpit', 'chase', 'orbit', 'wing', 'tower', 'flyby'];
export const VIEW_NAMES = {
  cockpit: 'コックピット Cockpit', chase: '追尾 Chase', orbit: '外部 Orbit', wing: '客室窓 Cabin window',
...
export class CameraRig {
  constructor(camera, dom, meta, world) {
    ...
    dom.addEventListener('pointermove', (e) => {
      ...
      this._drag = { x: e.clientX, y: e.clientY };
      if (this.view === 'cockpit' || this.view === 'wing') {
    ...
    dom.addEventListener('wheel', (e) => {
      ...
      if (this.view === 'cockpit' || this.view === 'wing' || this.view === 'tower' || this.view === 'flyby') {
  ...
  setView(v) {
    ...
    this.fov = v === 'cockpit' ? 62 : v === 'wing' ? 60 : v === 'tower' ? 30 : 55;
    this.flybyPos = null;
    this._init = false;
  }
  ...
  update(dt, fm, aircraftRoot) {
    ...
    switch (this.view) {
      ...
      case 'wing': {
        ...
        cam.position.copy(toWorld([-5.0, 0.6, -2.45]));
        const qy = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), this.yaw - 0.5);
        const qp = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1, 0, 0), this.pitch - 0.18);
