...
### 2b. 第2空港（`blender/build_airport2.py`）
...
* 出力：`web/assets/airport2.glb` と、スポット・看板位置・灯火・当たり判定の `airport2.json`（第2空港の中心を原点とする座標、シミュレーターが東 200 km に配置）
...
* **第2空港の地上車両・管制・町**：第2空港にも車両基地（トーイングカー・ケータリング・給油車・ベルトローダー）があり、駐機した AI 機へサービスに向かい、出発前にはトーイングカーがプッシュバック。第2空港の Tower / Ground との交信（着陸許可・地上走行・プッシュバック・離陸許可）。周囲には約 5,000 棟の町（街路・街灯・当たり判定付き、進入経路の下は空けてある）。
* **空港ごとに独立した無線**：ホーム空港と第2空港はそれぞれ別の Tower / Ground。無線は自機がいる（または向かっている、中間地点より東なら第2空港）空港の周波数に切り替わり（切替時に通知）、その空港の管制と交通だけが聞こえます。プレイヤーの要求（プッシュバック・タキシー・離陸・着陸・スポットへ）も、その空港の滑走路・誘導路（第2空港は Bravo・RWY09）・スポット・交通状況で処理され、第2空港でプレイヤーが滑走路の許可を得ている間は AI 機が離陸を待ちます。
...
python3 build_airport2.py               # 第2空港＋周囲の町
...
│   ├── build_airport2.py     第2空港と周囲の町（bpy）
