...
* **コックピット内部**：5 面の大型ディスプレイ（シム内の計器がリアルタイムに表示）、HUD コンバイナー、スラストレバー、操縦輪、座席。`blender/cockpit_b787.py` で計器盤とスイッチ類を作り込み：オーバーヘッドパネル（5×7 モジュールのトグル・ガード付きスイッチ・ノブ・点灯式押しボタン）、グレアシールドの MCP（数値窓・ノブ・モードボタン・A/P 解除バー）と EFIS パネル、マスターワーニング/コーション、ディスプレイ枠、脚レバー・オートブレーキ、ペデスタル（CDU ×2・フラップ/スピードブレーキレバー・燃料制御スイッチ・無線/トリム）、ティラー、酸素マスク箱、アーム付き EFB、パイロットシート。パネルの文字は夜間に発光（インテグラル照明）
...
* **視点**：コックピット（HUD）・追尾・外部・客室窓（窓側座席から翼のたわみが見える）・機内（通路）・管制塔・フライバイ
