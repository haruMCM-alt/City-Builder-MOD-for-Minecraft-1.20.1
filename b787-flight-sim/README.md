...
* **緊急事態（メーデー / パンパン）**（`playeratc.js`, `emergency.js`）：損傷・故障があると ATC ボタン（Y）が「メーデー宣言 / パンパン宣言」になり、宣言すると管制官が応答（スコーク 7700・最終進入へのレーダー誘導・即時着陸許可）、全局に「緊急事態発生・出発機は待機」を放送し、AI 機の離陸と着陸を止めて滑走路を空けます。飛行中の空港の消防署から赤い空港化学消防車 3 台（点滅灯つき）が滑走路脇の接地帯で待機、着陸後は機体を追って周囲を囲み、放水で火災を消します（約 15 秒）。火が消えて 45 秒後に「安全確認・滑走路再開」。宣言しないまま損傷があると 20 秒後に管制が「緊急事態を宣言しますか？」と確認、他空港に向かうと宣言が引き継がれ、主翼損傷や両発停止で低高度になると客室に「頭を下げて！」。**訓練用の故障**：Shift+E（ランダム）または一時停止画面のボタン（エンジン火災 / エンジン停止 / 燃料漏れ / **主翼脱落**（外翼30%が脱落・炎上） / **両エンジン停止**）。**消防・救助と脱出**（`emergency.js`、モデルは `blender/build_arff.py` → `arff.glb`：6輪の空港化学消防車〈キャブ屋根の放水銃・バンパー放水銃・赤青の回転灯・「空港消防 AIRPORT FIRE」の文字〉、耐熱服の消防隊員、乗客、2レーンの脱出スライド）：接地すると消防車は滑走路脇を全速力（約120 km/h）で機体に並走し、停止後は主翼端を回り込んで機首の左右前方と正面の攻撃位置へ。放水銃は放物線を描く水流で火点を狙い（火がなくても停止直後はブレーキに泡）、各車から隊員2名が降りて火点や脱出スライドの下へ歩いて向かいます。宣言していなくても損傷したまま着陸すると管制が消防を出します。緊急着陸で停止すると機長が脱出を指示し、火災と反対側の全ドアで脱出スライドが膨らみ、乗客がスライドを滑り降りて機体から約70 m離れた集合場所へ走ります。空港内で墜落した場合もクラッシュアラームで消防車が残骸に向かい放水します。**全自動緊急着陸**：宣言後にもう一度 Y（または Shift+L）で、乗員が最寄り空港の使用滑走路へ全自動で着陸（`AutoFlight.emergency`：エンジン火災チェックリストで消火、滑走路の手前に居れば直接 ILS、通り過ぎていればダウンウインド経由で 45° 会合、周辺の山は予測経路の地形から最低安全高度を計算して回避、主翼損傷時は最小操縦速度・フラップ制限・バンク 12° 制限・長めのファイナル・最大オートブレーキ）。メニューの自動操縦フライト中に故障が起きると乗員が自動で宣言して最寄り空港へ引き返します。**両エンジン停止時**（`AutoFlight.deadstick`）：スペースシャトルの着陸誘導と同じ「ヘディング整列円（HAC）」方式で滑空（新設の GLIDE モードで最良滑空速度 200 kt をピッチで保持、滑空比は約19）。最寄り空港の両方向・円の左右から、地形と建物を避けつつ届く組み合わせを選び、円に接線で入って旋回し滑走路延長線にぴったり整列、6.3°の急角度最終進入をフラップ20＋脚＋スピードブレーキで調整し、降下率に応じて早めに引き起こして接地します。高すぎれば最終進入を長くとりスピードブレーキを使います。滑空で届かないときは乗員がエンジン再始動を試み、1基が再始動したら片発の緊急着陸に切り替えます（燃料切れ・火災・脱落したエンジンは再始動不可）。**建物への衝突**：飛行中に翼端・エンジン・尾翼が建物に当たると（胴体が当たれば墜落）数秒で管制が「建物への衝突を確認、緊急事態を宣言しますか？」と呼び掛け、地上走行中に建物に接触するとグラウンドが停止を指示して消防車が点検に来ます。空港内（5 km 以内）で墜落した場合はクラッシュアラームが放送され、消防車が残骸へ急行して放水します。**AI 機の緊急事態**：時々到着機がパンパンを宣言（エンジン故障・油圧故障・客室の煙・バードストライク・急病人）して優先着陸し、同じく消防車が出動します。

  ![ARFF crash tender, fire fighter, passenger and escape slide (Blender)](docs/images/arff.jpg)
  ![Evacuation after an emergency landing](docs/images/evacuation.jpg)
...
node tests/emergency_land_test.mjs    # 緊急自動着陸 7 位置 × 4 故障 + 両エンジン停止 8 位置（AC=b737-800 / b767-300er も可）
```
